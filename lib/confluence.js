...
// Uses ancestor= scoping to walk under the transcripts parent — more reliable than global space search.
export async function findAllInsightPages(limit = 200) {
  const cqlAncestor = encodeURIComponent(`ancestor="${PARENT_ID}" AND title="${INSIGHT_PAGE_TITLE}" AND type=page`);
  const url = `${BASE_URL}/rest/api/content/search?cql=${cqlAncestor}&expand=body.storage,ancestors,history&limit=${Math.min(limit, 200)}`;
  const r = await fetch(url, { headers: authHeader() });
  if (!r.ok) {
