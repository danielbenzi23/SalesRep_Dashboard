...
// Uses ancestor= scoping under the transcripts parent + title starts-with match
// so we catch both the LEGACY "Claude Insights" and NEW "Claude Insights - <parentId>" pages.
export async function findAllInsightPages(limit = 200) {
  // CQL: title ~ "Claude Insights*" matches both legacy exact and the new suffixed pattern.
  // ~ is Confluence's fuzzy operator; wrapping in quotes with trailing * gives prefix match.
  ...
  const data = await r.json();
  // Client-side filter: title must actually start with the prefix (fuzzy ~ can match too broadly)
