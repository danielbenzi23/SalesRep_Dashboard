...
// Get a single buyer attribute by key
export async function getBuyerAttribute(buyerId, attribute) {
  return sb(`/api/external/buyer/${encodeURIComponent(buyerId)}/attribute/${encodeURIComponent(attribute)}`);
}

// Curated set of attributes most useful for higher-ed sales (DegreeSight context)
export const HIGHER_ED_ATTRIBUTES = [
  'Type', 'Url', 'City', 'StateCode',
  'HigherEdFullTimeEnrollment', 'HigherEdUndergradEnrollment', 'HigherEdGraduationEnrollment',
  'HigherEdControlOfInstitution', 'HigherEdLevelOfInstitution', 'HigherEdInstitutionType',
  'HigherEdMascot', 'HigherEdGraduationRate', 'HigherEdAcceptanceRate', 'HigherEdRetentionRate',
  'HigherEdSisModernizationStatus', 'Sis', 'LmsArray',
  'HigherEdEndowment', 'HigherEdNumberOfFullTimeEmployees',
  'PropensityToSpend', 'PropensityToSpendScore', 'PropensityToSpendSummary',
  'AiAdoptionScore', 'AiAdoptionSummary',
  'StartupFriendlinessScore',
  'ProcurementHellScore',
  'EnrollmentTrend', 'YoyChangeInEnrollment',
  'BudgetUrl'
];

// Fetch many attributes in parallel for one buyer. Returns object { attr: value }.
export async function getBuyerAttributes(buyerId, attrList = HIGHER_ED_ATTRIBUTES) {
  const results = await Promise.allSettled(
    attrList.map(a => getBuyerAttribute(buyerId, a))
  );
  const out = {};
  for (let i = 0; i < attrList.length; i++) {
    const r = results[i];
    if (r.status === 'fulfilled') {
      out[attrList[i]] = r.value.buyerData;
    }
  }
  return out;
}
